...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (isLoading) return;
    if (!isAccepted) {
      toast({ variant: "destructive", title: "تنبيه", description: "يرجى الموافقة على شروط الاستخدام للمتابعة." });
      return;
    }
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      const accounts = await provider.request({ method: 'eth_requestAccounts' })
        .catch((err: any) => {
          if (err.code === -32002) throw new Error("طلب اتصال معلق في MetaMask.");
          throw err;
        });
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-[440px] p-0 bg-[#0a0a0a] border-white/5 overflow-hidden rounded-[2.5rem] shadow-2xl" dir="rtl">
          <AnimatePresence mode="wait">
            {isLoading ? (
              ...
              <motion.div key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-8 space-y-8">
                ...
                <Tabs defaultValue="login" className="w-full">
                  ...
                  <TabsContent value="login" className="space-y-4">
                    <form onSubmit={handleLogin} className="space-y-4">
                      <Input type="email" placeholder="البريد الإلكتروني" className="h-14 bg-[#121212] border-white/5 rounded-xl font-bold text-right" value={loginData.email} onChange={(e) => setLoginData({...loginData, email: e.target.value})} required />
                      <Input type="password" placeholder="كلمة المرور" className="h-14 bg-[#121212] border-white/5 rounded-xl font-bold text-right" value={loginData.password} onChange={(e) => setLoginData({...loginData, password: e.target.value})} required />
                      <Button type="submit" className="w-full h-14 bg-primary font-black text-lg rounded-xl">تسجيل الدخول</Button>
                    </form>
                  </TabsContent>
                  ...
                  <TabsContent value="signup" className="space-y-4">
                    <form onSubmit={handleSignup} className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <Input type="password" placeholder="كلمة المرور" className="h-12 bg-[#121212] border-white/5 rounded-xl font-bold text-right text-xs" value={signupData.password} onChange={(e) => setSignupData({...signupData, password: e.target.value})} required />
                        <Input type="password" placeholder="تأكيد" className="h-12 bg-[#121212] border-white/5 rounded-xl font-bold text-right text-xs" value={signupData.confirmPassword} onChange={(e) => setSignupData({...signupData, confirmPassword: e.target.value})} required />
                      </div>
                      <div className="flex items-start gap-3 py-2 px-1">
                        <Checkbox id="terms" checked={isAccepted} onCheckedChange={(val) => setIsAccepted(val as boolean)} className="mt-1 border-white/20 data-[state=checked]:bg-primary" />
                        <label htmlFor="terms" className="text-[10px] leading-relaxed text-muted-foreground font-bold cursor-pointer">
                          أوافق على <button type="button" onClick={() => setPolicyConfig({ isOpen: true, type: 'terms' })} className="text-primary font-black">شروط الاستخدام</button> لمنصة EvenTix Chain.
                        </label>
                      </div>
